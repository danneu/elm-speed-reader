...
Elm.Belt.make = function (_elm) {
   ...
   $String = Elm.String.make(_elm);
   var _op = {};
   var targetValueInt = A2($Json$Decode.customDecoder,
   $Html$Events.targetValue,
   $String.toInt);
   var targetValueFloat = A2($Json$Decode.customDecoder,
   $Html$Events.targetValue,
   $String.toFloat);
   var chunkBy$ = F2(function (n,xs) {
      var chunk = A2($List.take,n,xs);
      return $List.isEmpty(chunk) ? _U.list([]) : A2($List._op["::"],
      chunk,
      A2(chunkBy$,n,A2($List.drop,n,xs)));
   });
   var chunkBy = function (n) {
      return function (_p0) {
         return $Array.fromList(A2(chunkBy$,n,$Array.toList(_p0)));
      };
   };
...
Elm.Machine = Elm.Machine || {};
Elm.Machine.make = function (_elm) {
   "use strict";
   _elm.Machine = _elm.Machine || {};
   if (_elm.Machine.values) return _elm.Machine.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Array = Elm.Array.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   ...
   var PromoteCandidateText = {ctor: "PromoteCandidateText"};
   var SetCandidateText = function (a) {
      return {ctor: "SetCandidateText",_0: a};
   };
   var SetWpm = function (a) {    return {ctor: "SetWpm",_0: a};};
   var SetChunkIdx = function (a) {
      return {ctor: "SetChunkIdx",_0: a};
   };
   var SetChunkSize = function (a) {
      return {ctor: "SetChunkSize",_0: a};
   };
   var view = F2(function (address,model) {
      ...
      _U.list([A2($Html.h1,
              ...
              _U.list([]),
              A2($Array.get,model.chunkIdx,model.chunks))))]))
              ,A2($Html.div,
              ...
              _U.list([A2($Html.input,
                      _U.list([$Html$Attributes.type$("range")
                              ...
                              ,$Html$Attributes.max($Basics.toString($Array.length(model.chunks) - 1))
                              ,$Html$Attributes.step("1")
                              ,$Html$Attributes.value($Basics.toString(model.chunkIdx))
                              ,A3($Html$Events.on,
                              "input",
                              $Belt.targetValueInt,
                              function (_p0) {
                                 return A2($Signal.message,address,SetChunkIdx(_p0));
                              })]),
                      _U.list([]))
                      ...
                      _U.list([A2($Html.button,
                              _U.list([A2($Html$Events.onClick,address,Toggle)
                                      ,$Html$Attributes.classList(_U.list([A2(_op["=>"],"btn",true)
                                                                          ,A2(_op["=>"],"btn-default",model.isRunning)
                                                                          ,A2(_op["=>"],"btn-success",$Basics.not(model.isRunning))]))]),
                              model.isRunning ? _U.list([A2($Html.span,
                                                        _U.list([$Html$Attributes.$class("rotate glyphicon glyphicon-refresh")]),
                                                        _U.list([]))
                                                        ,$Html.text(" Pause")]) : _U.list([$Html.text("Play")]))
                              ,$Html.text(" ")
                              ,A2($Html.button,
                      ...
                      _U.list([$Html.text(A2($Basics._op["++"],
                              $Basics.toString(model.wpm),
                              " WPM: "))
                              ,A2($Html.div,
                              _U.list([$Html$Attributes.$class("btn-group btn-group-xs")]),
                              _U.list([A2($Html.button,
                                      _U.list([$Html$Attributes.$class("btn btn-default")
                                              ,A2($Html$Events.onClick,address,SetWpm(model.wpm - 10))]),
                                      _U.list([$Html.text("Slower")]))
                                      ,A2($Html.button,
                                      _U.list([$Html$Attributes.$class("btn btn-default")
                                              ,A2($Html$Events.onClick,address,SetWpm(model.wpm + 10))]),
                                      _U.list([$Html.text("Faster")]))]))
                              ,$Html.text(A2($Basics._op["++"],
                              " @ ",
                              A2($Basics._op["++"],
                              $Basics.toString(model.chunkSize),
                              " per chunk: ")))
                              ,A2($Html.div,
                              _U.list([$Html$Attributes.$class("btn-group btn-group-xs")]),
                              _U.list([A2($Html.button,
                                      _U.list([$Html$Attributes.$class("btn btn-default")
                                              ...
                                              address,
                                              SetChunkSize(model.chunkSize - 1))]),
                                      _U.list([$Html.text("Smaller")]))
                                      ,A2($Html.button,
                                      _U.list([$Html$Attributes.$class("btn btn-default")
                                              ,A2($Html$Events.onClick,
                                              address,
                                              SetChunkSize(model.chunkSize + 1))]),
                                      _U.list([$Html.text("Larger")]))]))]))]))]))
              ,A2($Html.h3,_U.list([]),_U.list([$Html.text("Text")]))
              ,A2($Html.textarea,
              _U.list([$Html$Attributes.value(model.candidateText)
                      ,$Html$Attributes.$class("form-control")
                      ,A3($Html$Events.on,
                      "input",
                      $Html$Events.targetValue,
                      function (_p1) {
                         return A2($Signal.message,address,SetCandidateText(_p1));
                      })
                      ,$Html$Attributes.rows(6)]),
              _U.list([]))
              ,A2($Html.button,
              _U.list([$Html$Attributes.$class("btn btn-default")
                      ,A2($Html$Events.onClick,address,PromoteCandidateText)]),
              _U.list([$Html.text("Save")]))
              ,A2($Html.h3,_U.list([]),_U.list([$Html.text("State")]))
              ,A2($Html.p,
              _U.list([]),
              _U.list([$Html.text($Basics.toString(_U.update(model,
              {chunks: _U.list([_U.list(["<snip>"])])
              ,candidateText: "<snip>"
              ,text: "<snip>"})))]))
              ,A2($Html.footer,
              ...
              _U.list([$Html.text("Source code at ")
                      ,A2($Html.a,
                      _U.list([$Html$Attributes.href("https://github.com/danneu/elm-speed-reader")]),
                      _U.list([$Html.text("danneu/elm-speed-reader")]))]))]))]));
   });
   var Beat = function (a) {    return {ctor: "Beat",_0: a};};
   var wpmToInterval = F2(function (wpm,chunkSize) {
      return $Time.minute / ($Basics.toFloat(wpm) / $Basics.toFloat(chunkSize));
   });
   var init = function () {
      var text = initText;
      var chunkSize = 3;
      var wpm = 300;
      var interval = A2(wpmToInterval,wpm,chunkSize);
      return {ctor: "_Tuple2"
             ,_0: {interval: interval
                  ,wpm: wpm
                  ,prevBeat: $Maybe.Nothing
                  ,isRunning: false
                  ,text: text
                  ,chunks: A2($Belt.chunkBy,
                  chunkSize,
                  function (_p2) {
                     return $Array.fromList($String.words(_p2));
                  }(text))
                  ,chunkSize: chunkSize
                  ,chunkIdx: 0
                  ,wordIdx: 0
   ...
   var update = F2(function (action,model) {
      var _p3 = action;
      switch (_p3.ctor)
      {case "Toggle": return {ctor: "_Tuple2"
                             ,_0: _U.update(model,{isRunning: $Basics.not(model.isRunning)})
                             ,_1: model.isRunning ? $Effects.none : $Effects.tick(Beat)};
         case "Reset": return {ctor: "_Tuple2"
                              ...
                              ,wordIdx: 0})
                              ,_1: $Effects.none};
         case "SetChunkSize": var newChunkSize$ = A3($Basics.clamp,
           1,
           10,
           _p3._0);
           var newChunkIdx = model.wordIdx / newChunkSize$ | 0;
           var newInterval = A2(wpmToInterval,model.wpm,newChunkSize$);
           var newChunks = A2($Belt.chunkBy,
           newChunkSize$,
           function (_p4) {
              return $Array.fromList($String.words(_p4));
           }(model.text));
           return {ctor: "_Tuple2"
                  ,_0: _U.update(model,
                  ...
                  ,chunks: newChunks})
                  ,_1: $Effects.none};
         case "SetWpm": var wpm$ = A3($Basics.clamp,10,1000,_p3._0);
           var newInterval = A2(wpmToInterval,wpm$,model.chunkSize);
           return {ctor: "_Tuple2"
                  ,_0: _U.update(model,{interval: newInterval,wpm: wpm$})
                  ,_1: $Effects.none};
         case "SetCandidateText": return {ctor: "_Tuple2"
                                         ,_0: _U.update(model,{candidateText: _p3._0})
                                         ,_1: $Effects.none};
         case "PromoteCandidateText": var newChunks = A2($Belt.chunkBy,
           model.chunkSize,
           function (_p5) {
              return $Array.fromList($String.words(_p5));
           }(model.candidateText));
           return {ctor: "_Tuple2"
                  ,_0: _U.update(model,
                  {isRunning: false,chunks: newChunks,text: model.candidateText})
                  ,_1: $Effects.none};
         case "SetChunkIdx": var newChunkIdx$ = A3($Basics.clamp,
           0,
           $Array.length(model.chunks) - 1,
           _p3._0);
           return {ctor: "_Tuple2"
                  ,_0: _U.update(model,
                  {prevBeat: $Maybe.Nothing
           ...
           _v1_0: do {
              if (_p6._1.ctor === "Nothing") {
                    if (_p6._0 === false) {
                          ...
                          return {ctor: "_Tuple2"
                                 ,_0: _U.update(model,{prevBeat: $Maybe.Just(_p7)})
                                 ,_1: $Effects.tick(Beat)};
                       }
                 } else {
                    if (_p6._0 === false) {
                          ...
                          var newModel = function () {
                             if (_U.cmp(_p7 - _p6._1._0,model.interval) > -1) {
                                   var newChunkIdx = A2($Basics._op["%"],
                                   model.chunkIdx + 1,
                                   $Array.length(model.chunks));
                                   var newWordIdx = newChunkIdx * model.chunkSize;
                                   return _U.update(model,
                                   {prevBeat: $Maybe.Just(_p7)
                                   ,chunkIdx: newChunkIdx
                                   ,wordIdx: newWordIdx});
                                } else return model;
                          }();
                          return {ctor: "_Tuple2",_0: newModel,_1: $Effects.tick(Beat)};
                       }
                 }
           } while (false);
   ...
   return _elm.Machine.values = {_op: _op
                                ,Model: Model
                                ,wpmToInterval: wpmToInterval
                                ,init: init
                                ,Beat: Beat
                                ...
                                ,PromoteCandidateText: PromoteCandidateText
                                ,Toggle: Toggle
                                ,Reset: Reset
                                ,update: update
                                ,view: view
...
Elm.Main.make = function (_elm) {
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var app = $StartApp.start({update: $Machine.update
                             ,init: $Machine.init
                             ,view: $Machine.view
                             ,inputs: _U.list([])});
   var main = app.html;
